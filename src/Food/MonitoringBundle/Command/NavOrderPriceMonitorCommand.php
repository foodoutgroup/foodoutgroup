...
class NavOrderPriceMonitorCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
             ...
             * @var $orders Order[]
             */
            $orders = $em->getRepository('FoodOrderBundle:Order')->getCurrentNavOrders('-6 minute', true);
            ...
            if (!empty($orders) && count($orders) > 0) {
                ...
                foreach($orders as $order) {
                    ...
                    if (isset($navOrders[$order->getId()])) {
                        if (sprintf('%01.2f', $order->getTotal()) != sprintf('%01.2f', $navOrders[$order->getId()]['total'])) {
                            $criticalOrders[] = array(
                                'orderId' => $order->getId(),
