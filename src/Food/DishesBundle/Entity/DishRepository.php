...
class DishRepository extends EntityRepository
{
    ...
    public function getSmallestDiscountPrice($dishId)
    {
        ...
        return $qb->getQuery()->getResult();
        */
        $query = "SELECT IF(discount_price = 0 OR discount_price IS NULL, price, discount_price) as discountPrice FROM dish_size WHERE dish_id=".intval($dishId)." ORDER BY IF(discount_price = 0, price, discount_price) ASC";
    ...
    public function getLargestDiscountPrice($dishId)
    {
        ...
        return $qb->getQuery()->getResult();
        */
        $query = "SELECT IF(discount_price = 0 OR discount_price IS NULL, price, discount_price) as discountPrice FROM dish_size WHERE dish_id=".intval($dishId)." ORDER BY IF(discount_price = 0, price, discount_price) DESC";
        $stmt = $this->getEntityManager()->getConnection()->prepare($query);
        $stmt->execute();
        return $stmt->fetchAll();
    }
