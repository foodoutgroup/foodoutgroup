...
change_location = function (element, click_callback, data_callback, change_text, cancel_text, request_url) {
    ...
    var change_options = {
        ...
        click: function () {
            var dialog = $(this);
            $.ajax({
                type: 'GET', url: request_url, data: data_callback(), success: function (response) {
...
(function (form) {
    ...
    button_do_pickup.click(function (e) {
        ...
        $.get($(this).data("url"), {
            "redirect": true,
            "type": "pickup",
            "address": input_auto_complete.data("selected")
        }, function (response) {
            if (response.success) {
...
function badgeFunction(selector) {

    $.ajax({
        url: $(selector).attr('data-url'),
        type: 'POST',
        data: {
            address: $(selector).attr('data-code'),
            type: 'badge'
        },
